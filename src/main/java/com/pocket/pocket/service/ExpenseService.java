    ...
    public void manageBudget(Expense expense) {
        ...
        switch (category) {
            ...
            case "Bills":
                ...
                break;
            case "Rent":
                budget.setRent(budget.getRent() - amount);
                break;
            case "Medical":
                budget.setMedical(budget.getMedical() - amount);
                break;
            case "Entertainment":
                budget.setEntertainment(budget.getEntertainment() - amount);
                break;
            case "Shopping":
                budget.setShopping(budget.getShopping() - amount);
                break;
            case "Personal":
                budget.setPersonal(budget.getTravel() - amount);
                break;
            case "Travel":
                budget.setTravel(budget.getTravel() - amount);
                break;
            case "Other":
                budget.setOther(budget.getOther() - amount);
                break;
            default:
